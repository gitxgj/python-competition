  ...
  <t-card theme="poster2">
    ...
    <template #status>
      <t-tag :theme= 'success' :disabled="false">{{
        product.pass===0?'未通过':'已通过'
      }}</t-tag>
    </template>
    <template #content>
      <p class="list-card-item_detail--name">{{ product.competitionSchemeId }}</p>
      <p class="list-card-item_detail--name">{{ '得分：'+ product.competitionScore }}</p>
      <p class="list-card-item_detail--desc" style="color: #10e51a" v-if="product.pass===1">{{ '得分已转化成相应积分' }}</p>
      <p class="list-card-item_detail--desc" style="color: #d70c1d" v-if="product.pass===0">{{ '比赛未通过，未获得积分' }}</p>
      <p class="list-card-item_detail--desc">{{
            '考试时间：'+product.jionTime
        }}</p>
    </template>
    <template #footer>
...
<script lang="ts">
import { ShopIcon, CalendarIcon,CheckCircleIcon,CloseCircleIcon, ServiceIcon, UserAvatarIcon, LaptopIcon, MoreIcon, AddIcon } from 'tdesign-icons-vue';

export default {
  name: 'ListCard',
  components: {
    ShopIcon,CheckCircleIcon,CloseCircleIcon,
    CalendarIcon,
    ServiceIcon,
    UserAvatarIcon,
    LaptopIcon,
