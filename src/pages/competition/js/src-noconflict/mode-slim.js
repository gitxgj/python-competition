ace.define(
  ...
  ['require', 'exports', 'module', 'ace/config', 'ace/lib/oop', 'ace/mode/text_highlight_rules'],
        ...
        start: [
          {
            ...
            onMatch: function (value, state, stack, line) {
              ...
              var language = m[2];
              if (!/^(javascript|ruby|coffee|markdown|css|scss|sass|less)$/.test(language)) language = '';
                ...
                onMatch: function (value, state, stack, line) {
                  ...
                  if (modes[language]) {
                    var data = modes[language]
                      .getTokenizer()
                      .getLineTokens(line.slice(indent.length), embedState.slice(0));
                    stack[1] = data.state;
                    return data.tokens;
                  }
                  return this.token;
      ...
      this.$rules = {
        start: [
          ...
          },
          {
            token: 'constant.numeric',
            regex:
              numRe + '(?:ch|cm|deg|em|ex|fr|gd|grad|Hz|in|kHz|mm|ms|pc|pt|px|rad|rem|s|turn|vh|vmax|vmin|vm|vw|%)',
          },
          {
            token: 'constant.numeric',
            regex: '#[a-f0-9]{6}',
          ...
          },
          {
            token: function (value) {
              ...
              else if (constants.hasOwnProperty(value)) return 'constant.language';
              else if (functions.hasOwnProperty(value)) return 'support.function';
              else if (colors.hasOwnProperty(value.toLowerCase())) return 'support.constant.color';
              else if (tags.hasOwnProperty(value.toLowerCase())) return 'variable.language';
              else return 'text';
            },
            regex: '\\-?[@a-z_][@a-z0-9_\\-]*',
          },
...
ace.define(
  ...
  ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/scss_highlight_rules'],
  function (require, exports, module) {
    'use strict';
    var oop = require('../lib/oop');
    var lang = require('../lib/lang');
...
ace.define(
  ...
  ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text_highlight_rules', 'ace/mode/css_highlight_rules'],
  function (require, exports, module) {
    'use strict';
    var oop = require('../lib/oop');
    var TextHighlightRules = require('./text_highlight_rules').TextHighlightRules;
            ...
            switch (token.value) {
              case 'do':
                for (var i = stream.$tokenIndex - 1; i >= 0; i--) {
                  ...
                  if (
                    prevToken &&
                    (prevToken.value == 'while' || prevToken.value == 'until' || prevToken.value == 'for')
                  ) {
                    level = 0;
                    break;
                  }
